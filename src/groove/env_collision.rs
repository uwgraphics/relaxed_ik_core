...
impl CollisionObjectData {
    pub fn new(is_link: bool) -> CollisionObjectData {
        Self {
            is_link: is_link,
        }
    }
...
pub fn init_collision_world(
...
) -> (CollisionWorld<f64, CollisionObjectData>, Vec<CollisionObjectSlabHandle>) {
    ...
    let link_data = CollisionObjectData::new(true);
    let plane_data = CollisionObjectData::new(false);
    let sphere_data = CollisionObjectData::new(false);
    ...
    for arm_idx in 0..frames.len() {
        ...
        for i in 0..last_elem {
            ...
            let handle = world.add(segment_pos, segment, link_groups, proximity_query, link_data.clone());
...
pub fn update_collision_world(
    ...
    for i in 0..length - 1 {
        ...
        let end_pt = Point3::from(frames[arm_idx].0[i + 1]);
        let segment = ShapeHandle::new(Segment::new(start_pt, end_pt));
        let co = world.objects.get_mut(link_handles[arm_idx * length + i]).unwrap();
        co.set_shape(segment);
    }
}

pub fn handle_proximity_event<'a>(
    world: &'a CollisionWorld<f64, CollisionObjectData>,
    event: &ProximityEvent<CollisionObjectSlabHandle>,
) -> Option<&'a CollisionObject<f64, CollisionObjectData>> {
    if event.new_status == Proximity::WithinMargin {
        println!("WithinMargin");
        let c1 = world.objects.get(event.collider1).unwrap();
        let c2 = world.objects.get(event.collider2).unwrap();
        if !c1.data().is_link {
            return Some(c1);
